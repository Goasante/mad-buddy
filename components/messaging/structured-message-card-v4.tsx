      ...
      <div>
        <div className={cardClass}>
          <div className="flex items-start gap-3">
            <span className={cn("grid h-10 w-10 shrink-0 place-items-center rounded-full", mine ? "bg-white/12" : "bg-primary/10 text-primary")}><UserRound className="h-4.5 w-4.5" /></span>
            <div className="min-w-0 flex-1"><span className="text-xs font-medium opacity-60">Contact</span><strong className="block truncate text-sm">{payload.displayName}</strong>{payload.organization ? <span className="mt-0.5 flex items-center gap-1 truncate text-xs opacity-65"><Building2 className="h-3 w-3" />{payload.organization}</span> : null}</div>
          ...
          {copied ? <span className="mt-2 block text-xs font-medium opacity-65">Copied</span> : null}
        </div>
  ...
  if (payload.kind === "place") {
    ...
    return (
      <div>
        <div className={cardClass}>
          <div className="flex items-start gap-3">
            <span className={cn("grid h-10 w-10 shrink-0 place-items-center rounded-2xl", mine ? "bg-white/12" : "bg-primary/10 text-primary")}><MapPin className="h-4.5 w-4.5" /></span>
            <div className="min-w-0 flex-1"><span className="text-xs font-medium opacity-60">{payload.placeKind === "area" ? "Area" : "Place"}</span><strong className="block truncate text-sm">{payload.placeName}</strong>{payload.areaLabel ? <span className="mt-0.5 block truncate text-xs opacity-65">{payload.areaLabel}</span> : null}{payload.addressLabel ? <span className="mt-1 block line-clamp-2 text-xs leading-relaxed opacity-65">{payload.addressLabel}</span> : null}</div>
          ...
          {copied ? <span className="mt-1 block text-xs font-medium opacity-65">Copied</span> : null}
        </div>
        <MessageRetentionV4 conversationId={conversationId} messageId={messageId} mine={mine} />
      </div>
    );
