...
type UserOperation = "disable" | "enable" | "pause_visibility" | "clear_notification_badge" | "reset_glow_signal";

export function AdminUserControls({ userId, disabled, canQuickFix }: { userId: string; disabled: boolean; canQuickFix: boolean }) {
  ...
  function apply() {
    startTransition(async () => {
      const result = operation === "disable" || operation === "enable"
  ...
  return (
    <details className="group md:col-span-5">
      ...
      <div className="mt-3 grid gap-2 rounded-xl border border-border/70 bg-secondary/20 p-3 sm:grid-cols-[minmax(170px,220px)_minmax(220px,1fr)_auto] sm:items-start">
        ...
        <Input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Reason for this action" maxLength={300} disabled={pending} aria-label="Reason for account action" />
