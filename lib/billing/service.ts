...
export async function loadBillingState(admin: Admin, userId: string): Promise<BillingState> {
  ...
  const base = {
    trialId: null, trialPlan: null, trialStartedAtMs: null, trialEndsAtMs: null,
    earnedRewardId: null, earnedPlan: null, earnedStartsAtMs: null, earnedEndsAtMs: null, earnedGraceEndsAtMs: null
  } as const;
  if (!data) return { plan: "free", status: "free", periodEndMs: null, graceEndsMs: null, ...base };
  return {
...
export async function loadEffectivePlansForUsers(
  ...
  if (subscriptionsResult.error) return new Map(uniqueIds.map((userId) => [userId, "free" as const]));
