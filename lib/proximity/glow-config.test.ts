 ...
 *
 * The prototype at `design-reference/proximity-glow-v1.html` is
...
const PROTOTYPE = readFileSync("design-reference/proximity-glow-v1.html", "utf8");
...
describe("the Glow reads on both themes", () => {
  const css = readFileSync("app/globals.css", "utf8");

  it("deepens the pigment on light backgrounds", () => {
    // The prototype is designed on near-black. Mad Buddy's DEFAULT theme is
    // cream, where the prototype's brighter warmth loses most of its contrast
    // and the weakest states vanish. The base rule therefore ships a darker,
    // more saturated orange.
    const base = css.slice(css.indexOf(".proximity-glow {"));
    expect(base.slice(0, 200)).toContain("--glow-brand: 214 118 26");
  });

  it("restores the prototype's exact palette in dark mode", () => {
    const dark = css.slice(css.indexOf("html.dark .proximity-glow {"));
    expect(dark.slice(0, 200)).toContain("--glow-brand: 232 140 43");
    expect(dark.slice(0, 200)).toContain("--glow-highlight: 255 176 78");
  });

  it("changes only colour, never the values that carry the hierarchy", () => {
    // A theme may repaint the Glow. It may NOT retune it: the moment a theme
    // sets a radius, strength, blur or pulse, the two themes stop agreeing
    // about what a state means.
    const dark = css.slice(
      css.indexOf("html.dark .proximity-glow {"),
      css.indexOf("html.dark .proximity-glow {") + 300
    );
    for (const forbidden of ["--glow-ring", "--glow-outer", "--glow-blur", "--glow-strength", "--glow-pulse"]) {
      expect(dark, forbidden).not.toContain(forbidden);
    }
  });
});
