/**
 * Terms of Service content, shared by every platform.
 *
 * Extracted from app/terms/page.tsx so the native app can render the SAME
 * document rather than a copy. Two copies of a legal document is two documents
 * that will eventually disagree, and the one a user consented to would become
 * a question of which build they were running.
 */
import { legalContact } from "@/content/privacy-policy";

export const TERMS_EFFECTIVE_DATE = "23 July 2026";

export type TermsBlock = { type: "paragraph"; text: string } | { type: "list"; items: string[] };
