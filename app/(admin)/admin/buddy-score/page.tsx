...
export default async function AdminBuddyScorePage() {
  ...
  const [result, rewardsResult] = await Promise.all([
    admin.from("buddy_score_ledger").select("id,user_id,event_type,points_delta,source_reference,rule_version,created_at").order("created_at", { ascending: false }).limit(100),
  ...
  return <div className="space-y-7">
    ...
    <AdminSection title="Score ledger" description="Recent versioned events from trusted application activity.">
      {result.error ? <AdminQueryError /> : rows.length === 0 ? <AdminEmptyState icon={Activity} title="No score activity" description="Trusted score events will appear after users complete eligible activity." /> : <Card className="divide-y divide-border/70 overflow-hidden p-0">{rows.map((row) => { const profile = profileMap.get(row.user_id); return <div key={row.id} className="grid gap-3 px-4 py-3.5 md:grid-cols-[minmax(0,1fr)_auto_auto] md:items-center"><div className="min-w-0"><p className="truncate text-sm font-medium">{profile?.full_name ?? "Mad Buddy user"} <span className="text-muted-foreground">@{profile?.username ?? row.user_id.slice(0, 8)}</span></p><p className="mt-1 break-all text-xs text-muted-foreground">{humanizeAdminValue(row.event_type)} / {row.source_reference} / rule v{row.rule_version} / {formatAdminDate(row.created_at, true)}</p></div><AdminStatus label={`Rule v${row.rule_version}`} tone="default" /><span className={row.points_delta >= 0 ? "text-sm font-semibold text-emerald-400" : "text-sm font-semibold text-red-400"}>{row.points_delta > 0 ? "+" : ""}{row.points_delta}</span></div>; })}</Card>}
    </AdminSection>
