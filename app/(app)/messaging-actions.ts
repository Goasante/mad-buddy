...
import type { MessageReactionType, QuickActionType } from "@/lib/supabase/database.types";
...
const sendSchema = z.object({
  ...
  /** Client-generated idempotency key so a retry can't double-send (§7). */
...
/** Notifies members, honoring each recipient's preview privacy (spec §56). */
async function notifyOtherMembers(
  ...
  const recipients = (members ?? []).filter(
    (member) => !member.muted_until || member.muted_until < nowIso
  );
...
export async function getConversationsAction(): Promise<ConversationView[]> {
  ...
  for (const conversation of conversations ?? []) {
    ...
    const { count: unread } = await admin
      ...
      .gt(
        "created_at",
        membership?.last_read_message_id ? await readTimestampFor(admin, membership.last_read_message_id) : new Date(0).toISOString()
      );
...
async function readTimestampFor(
  admin: ReturnType<typeof createSupabaseAdminClient>,
  messageId: string
): Promise<string> {
...
export async function getMessagesAction(conversationId: string): Promise<ChatMessageView[]> {
  ...
  if (!access.canView) return []; // Never serve a guessed conversation id (§42).

  const { data: messages } = await admin
    .from("messages")
    .select(
      "id, sender_id, message_type, text_content, quick_action_type, status, created_at, edited_at, deleted_at"
    )
    .eq("conversation_id", conversationId)
...
export async function markConversationReadAction(conversationId: string): Promise<MessagingActionState> {
  ...
  // Only mark others' messages read when this user allows read receipts (§10).
